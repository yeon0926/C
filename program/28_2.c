...
main()
{
	...
	while(1) {
		...
		/* sercSpclfd 소켓으로 들어오는 클라이언트와 요청 받아들임*/
		...
		/*자식 프로세스는*/
			case 0: 
				...
				/*임의의 클라이언트에 대해 반복*/
				while(1) {
					/*ckbtSockfd 소켓으로 들어오는 데이터를 받아 recvBuffer에 저장*/
			...
			default:
				while(1) {
					...
					if(send(clntSockfd, recvBuffer, strlen(sendBuffer), MSG_NOSIGNAL) != strlen(sendBuffer)){
