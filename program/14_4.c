#include <stdio.h>
#include <time.h> /*time, localtime, asctime, strftime함수가 정의된 헤더 파일*/
#define SIZE 124
...
main()
{
	...
	time(&time_now);
	/*time_t형 시간 정보 time_now를 struct tm형 지역시간 정보로 변환해서 tm_local에 저장*/
	tm_local = localtime(&time_now);

	/*struct tm형 시간 정보 tm_local을 문자열로 변환해서 출력 */
	printf("%s", asctime(tm_local));

	/*struct tm 형 신간 정보 tm_local을 서식을 갖춘 시간 정보로 변환하는데,
